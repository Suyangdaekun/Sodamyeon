...
- Mediapipe의 400개 이상 포인트 모델을 사용한 고급 얼굴 랜드마크 감지
- 다양한 랜드마크 감지 시스템의 종합적인 벤치마킹을 통한 모델 선택
- 전통적인 동양 관상학 원리에 기반한 관상 분석 알고리즘
...
### 기술적 구현
- **종합적인 벤치마킹**: Mediapipe, dlib, FAN을 평가하여 최고 성능의 모델 선택
- **Mediapipe 선택**: 400개 이상의 얼굴 랜드마크로 우수한 성능을 제공하여 선택됨
- **고밀도 랜드마크 모델**: 다음을 포함한 상세한 얼굴 특징을 식별합니다:
  - 턱선 윤곽
  - 눈썹 모양
  - 코 구조
  - 눈 윤곽
...
- **실시간 처리**: 초당 30+ 프레임의 성능으로 안드로이드 모바일 기기에 최적화

### 관상 분석
모델은 다음을 포함한 주요 측정값과 비율을 추출합니다:
- 얼굴 형태 분류 (타원형, 둥근형, 사각형, 하트형 등)
- 얼굴 삼분법 및 황금비율 측정
- 눈과 눈 사이 거리 비율
- 코와 입술 비율
- 얼굴 대칭 분석
...
이러한 측정값은 전통적인 동양 관상학 원리를 참조하는 독자적인 알고리즘을 통해 처리되어 종합적인 성격 분석과 호환성 프로필을 생성합니다.
